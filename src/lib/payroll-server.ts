...
export async function buildCalcInputs(
  ...
  const countryCode = employee.organization.countryCode
  const ruleRows = await prisma.countryPayrollRule.findMany({
    where: { organizationId: employee.organizationId, countryCode },
    include: {
  ...
  const rules = ruleRows.map(ruleFromDb)
...
export async function generatePayslipDraft(
  ...
  const { inputs, countryCode, currency, organizationId } = await buildCalcInputs(employeeId, period, opts)
  const draft = calculatePayslip(inputs)
  return { draft, countryCode, currency, organizationId }
}
